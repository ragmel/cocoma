...
def main():
    ...
    if len(arguments)>0:
        ...
        if options.xml and not options.emuNow:  
                ...
                (emulationName,emulationType,emulationLog, resourceTypeEmulation, startTimeEmu,stopTimeEmu, distroList) = XmlParser.xmlReader(arguments[0])
                if startTimeEmu.lower() =="now":
                    startTimeEmu1 = EmulationManager.emulationNow()
                    EmulationManager.createEmulation(emulationName, emulationType,emulationLog, resourceTypeEmulation, startTimeEmu1,stopTimeEmu,distroList)
                else:
                    EmulationManager.createEmulation(emulationName, emulationType,emulationLog, resourceTypeEmulation, startTimeEmu,stopTimeEmu,distroList)
