...
def cpuLoad(distributionID,runNo,cpuUtil,ncpus,duration):
    ncpus = int(ncpus)
    print "ncpus before: ", ncpus
    if ncpus == 0 :
        ncpus = psutil.NUM_CPUS
...
#         runLookbusyPidNo =runLookbusy.pid
    ...
    print "ncpus after: ", ncpus
    
    for n in xrange(0, ncpus):
        print "taskset", "-c", str(n), "lookbusy", "-c",str(cpuUtil),"-n", str(1)
        runLookbusy = subprocess.Popen(["taskset", "-c", str(n), "lookbusy", "-c",str(cpuUtil),"-n", str(1),"&"])
        print "runLookbusy.stdout: ", runLookbusy.stdout
        runLookbusyPidNo =runLookbusy.pid
        #cat /proc/27041/status
        cmd="cat /proc/"+str(runLookbusy.pid)+"/status"
        os.system(cmd)
        print "Started lookbusy on PID No: ",runLookbusyPidNo
